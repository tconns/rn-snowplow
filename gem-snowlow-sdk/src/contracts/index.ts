...
export interface IMediaAnalytics {
    ...
    trackPictureInPictureChange(isPip: boolean): void;
    trackPercentProgress(percent: number): void;
    
    trackAdStart(adId: string, adType: string, duration: number, skipOffset?: number): void;
    trackAdEnd(): void;
    trackAdBreakStart(breakId: string, breakType: string, startTime: number): void;
...
export interface IAppAnalytics {
    ...
    trackSelfDescribingEvent(schema: string, data: Record<string, unknown>): void;
    trackSiteSearch(terms: string[], filters?: Record<string, unknown>, totalResults?: number): void;
    setGlobalContext(context: Record<string, unknown>): void;
}
